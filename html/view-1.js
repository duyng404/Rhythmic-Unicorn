...
function addResult(count, title, artist, album, img, preview, spotId){
	...
	<div class="song" id="song-'+count+'">\
		...
		<button class="select-button">View</button>\
		<div class="select-overlay">Viewing...</div>\
	</div>');
...
function assignBehaviorForResults(path){
	...
	$(path+' .select-button').click(function(){
		...
		if (balance >= 100){
			$.ajax({
				url: '/api/view/'+data.spotId,
				type: "GET",
				success: function(data){
					balance -= 100;
					localStorage.setItem('balance',balance);
					localStorage.setItem('viewSong',JSON.stringify(data));
					showResult('success');
				}
			});
		} else {
...
function showResult(aa){
	if (aa == 'success'){
		window.location.href = "/view-2.html";
	} else {
